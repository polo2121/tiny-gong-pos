...
const BubbleButton = ({ name }: { name: ButtonName }) => {
  ...
  return (
    <a
      href="/sales"
      className={`${buttonType[name]?.fillColor} ${buttonType[name]?.strokeColor} ${buttonType[name]?.dropShadowColor} flex flex-col gap-2 justify-end w-full h-44 px-4 py-6 rounded-home text-white relative border overflow-hidden group focus-visible:none will-change-transform transition-transform select-none touch-none animate-(--anim-bubble) active:animate-(--anim-bubble-press) bubble-button`}
