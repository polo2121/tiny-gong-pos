...
import { StripeShape } from "./SVG/StripeShape";
  ...
  "flex h-48 flex-col justify-end rounded-[36px] border-5 border-[#EEEEEE] px-5 py-6 text-white shadow-[inset_0_8px_0_0_rgba(255,255,255,0.25)] transition-colors  relative group bubble-press active:scale-95 btn-press",
  {
    variants: {
      variant: {
        ...
        profits: "bg-theme-profits ",
        expenses: "bg-theme-expenses ",
        customers: "bg-theme-customers ",
      },
    },
    defaultVariants: {
      variant: "default",
...
const NavigationLink = ({
  ...
  return (
    ...
    >
      {icon ? <span className="shrink-0 relative z-20">{icon}</span> : null}
      <span className="flex flex-col gap-3 relative z-20">{children}</span>
      <StripeShape className="relative left-48 bottom-18 transition-transform duration-300 ease-out group-hover:-translate-x-full bg-amer-900 h-80 z-10" />
